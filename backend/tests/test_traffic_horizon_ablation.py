...
class TrafficHorizonAblationTests(unittest.TestCase):
    ...
    def test_only_traffic_receives_a_ninety_second_horizon(self) -> None:
    ...
    def test_schema_twelve_refuses_a_schema_eight_traffic_checkpoint(self) -> None:
        env = self.traffic.make_env(False)
        agent = PPOAgent(
    ...
    def test_protocol_v12_records_the_exact_task_horizon(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
