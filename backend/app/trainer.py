...
class Trainer:
    ...
    def _save_checkpoint(self) -> None:
        ...
        try:
            ...
            # Gate transitions and persistence form one transaction. Restore
            # both curriculum layers and all random streams on any failure.
    ...
    def _save_checkpoint_transaction(self, rng_state: dict):
        """Evaluate, advance nested/outer gates, and durably save atomically."""
        control_result = None
        try:
        ...
        if control_result is not None:
            ...
            if not callable(record_control) or control is None:
                raise TypeError(
                    "scenario training-control contract is incomplete")
        ...
        # The outer cp11 gate is observable only after the nested transition
        # has established canonical 30 m/s proficiency.
